<template>
    ...
    >
        ...
        <div @click.self="showPicker = false" v-show="showPicker"
             style="top:0; position:fixed; z-index:1; width: 100vw; height: 100vh; background-color: transparent;"></div>
        <transition name="slide">
            <date-picker locale="th" v-show="showPicker" v-model='pickerDate' mode="range" is-inline is-dark/>
        </transition>
    ...
    export default {
        ...
        mounted() {
            ...
            this.$store.commit('updateForm', {
                ...
                'value': (this.pickerDate) ? [moment(this.pickerDate[0]).toISOString(), moment(this.pickerDate[1]).toISOString()] : null,
            });
        ...
        watch: {
            ...
            pickerDate(value) {
                ...
                this.dateString = moment(value['start']).format('D MMM YYYY') + ' - ' + moment(value['end']).format('D MMM YYYY');
                this.value = value;
            ...
            value: {
                ...
                set(value) {
                    ...
                    this.$store.commit('updateForm', {
                        ...
                        'field': this.field,
                        'subfield': this.subfield,
                        'value': [moment(value['start']).startOf('day').format('YYYY-MM-DD HH:mm:ss'), moment(value['end']).endOf('day').format('YYYY-MM-DD HH:mm:ss')]
                    });
            ...
            checkValidateError: function () {
                ...
                if (this.$v.value.$invalid) {
                    ...
                    this.errorsToCheck.some((param) => {
                        if (param === 'required') {
                            return this.error = 'Please select a date.';
                        }
                    });
                } else {
                    if (this.formError) {
