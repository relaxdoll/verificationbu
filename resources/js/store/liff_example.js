...
export default {
    ...
    actions: {
        ...
        getProfile({commit}) {
            if (liff.isLoggedIn()) {
                ...
                commit('getUserId', 'U3094f16f5d2775edcaebca950e013091');
                // commit('getUserId', 'U03851075bfe338576ba62957198a96b0');
            }
        },
        getUserId({commit}, lineId) {
            return new Promise((resolve, reject) => {
                axios.get(`api/line/user/crud/getRoleDetailByLineId/${lineId}`, {})
                    .then(response => {
                        commit('saveDriver', response.data.data.driver);
                        commit('saveApprover', response.data.data.approver);
                        commit('saveTechnician', response.data.data.technician);
    ...
    mutations: {
        ...
        getUserId(state, lineId) {
            this.dispatch('getUserId', lineId);
        },
        ...
        saveApprover(state, approver) {
            if (approver) {
                state.approver_id = approver.id;
                state.approver = approver;
            }

            if (state.profile.pictureUrl) {
                if (approver.avatar !== state.profile.pictureUrl) {
                    this.commit('updateAvatar');
                }
            }
        },
        saveTechnician(state, technician) {
            if (technician) {
                state.technician_id = technician.id;
                state.technician = technician;
            }

            if (state.profile.pictureUrl) {
                if (technician.avatar !== state.profile.pictureUrl) {
                    this.commit('updateAvatar');
                }
            }
        },
        ...
        setProfile(state, profile) {
            this.commit('getUserId', profile.userId);
