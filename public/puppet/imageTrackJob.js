...
async function forward() {
    ...
    await page.goto('https://maintenance.eecl.co.th/imageTrackJob');
    ...
    async function sendFlex(job) {
        ...
        let groupElement = await page.$(".customer" + job);

        let groupName = await (await groupElement.getProperty('textContent').catch(async (e) => {

            await closeNode();

        })).jsonValue();

        let groupElement2 = await page.$(".group" + job);

        let groupName2 = await (await groupElement2.getProperty('textContent').catch(async (e) => {

            await closeNode();

        })).jsonValue();

        console.log('group name: ' + groupName + '&' + groupName2);
        ...
        await searchGroup(newPage, groupName2, true);

        await selectGroup(newPage, groupName2);
